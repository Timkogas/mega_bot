...
class TelegramBotApp {
    ...
    private async _sendMessageOnTaskFive(chatId: number, dbUser: IUserDb): Promise<void> {
        try {
            ...
            const text = `<b>Задание #5. Сходите в магазин с шоппером и загрузите чек на N рублей без позиции «пакет»</b>\n\n🛍️ Собрался(ась) за покупками? Мы помогаем сделать шаг навстречу экологичному образу жизни — получи от МЕГИ в подарок шоппер на столе информации, посети любой магазин и загрузи чек без пакета, чтобы получить баллы. <i>Шоппер можно получить только при условии выполнения минимум 3-х заданий</i>.\n\nЕсли ты действительно хочешь вносить свой вклад в экологию и приобщиться к осознанному потреблению, то использование качественного шоппера вместо пакета из пластика — это хорошее начало! ⚡\n\n<i>P.S.: Чеки из магазинов можно сканировать неограниченное количество раз. Как только отсканируешь все-все чеки, жми на кнопку «Завершить задание»</i>`
